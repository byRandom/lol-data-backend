...
const getSummoner = async (req, res) => {
  ...
  let amount = 20;
  let start = 0;
  if (req.params.amount) amount = req.params.amount;
...
const saveMatchDetails = async (matchId) => {
  try {
    ...
    if (!matchDetailsStored) {
      return {
        message: "No se ha podido guardar el matchDetails en la base de datos",
      };
    }
    return { matchDetails: matchDetailsStored };
  } catch (error) {
    return {
      message: "Error al guardar el matchDetails en la base de datos",
      error,
    };
  }
};
...
const getMatchDetailsFromDatabase = async (matchId) => {
  try {
    const matchDetailsStored = await matchDetails
    ...
    try {
      ...
      for (let i = 0; i < matchListData.length; i++) {
        ...
        if (!matchInDB) {
          ...
          let matchDetailsData = await getMatchDetailsFromDatabase(
            matchListData[i]
          );
